...
while cap.isOpened():
    ...
    # Read light status from Arduino
    if ser.in_waiting > 0:
        light_status = ser.readline().decode('utf-8').strip()
    ...
    current_red_color = green_color if light_status == "green" else (yellow_color if light_status == "yellow" else red_color)
